...
4. **Runtime Operations:**
   ...
   - **Sidebar Interaction:** The custom sidebar handles project creation, editing, deletion, conversation renaming, conversation reassignment between projects, and delegates mapped chat navigation to the equivalent native Gemini chat entry when available. Unmapped native chats are hidden from the native list via `hideMappedChats()`.
...
- **UI Metadata:** Exports the predefined `ICONS` list used by the project create/edit modals, including general-purpose and category-specific icons for technology, finance, games, books, health, browsers, and programming.
...
- **Details:** 
  ...
  - Binds event listeners for custom UI buttons such as edit, delete, the conversation actions menu, and toggle collapse.
